...
export class SmartContractUtil {
    ...
    public static async getConnectionProfile() {
        ...
        const connectionProfilePath: string = path.join(homedir, '.fabric-vscode', 'environments', 'g', 'gateways', 'Org1', 'Org1.json');
    ...
    public static async submitTransaction(contractName: string, functionName: string, args: string[], gateway): Promise<Buffer> {
        ...
        if (contractName !== '') {
            contract = await network.getContract('vulnerability-private-data-collections', contractName);
        } else {
            contract = await network.getContract('vulnerability-private-data-collections');
        }
